...
class NMDbusInterface(object):
    ...
    def __new__(cls, object_path=None):
        ...
        if object_path and not cls.introspection_data:
            ...
            cls.introspection_data = proxy.Introspect(
                dbus_interface="org.freedesktop.DBus.Introspectable"
            )
            ...
            for element in root:
                if (
                    element.tag == "interface"
                    and element.attrib["name"] in cls.interface_names
                ):
                    for item in element:
                        if item.tag == "property":
                            setattr(
                                cls,
                                item.attrib["name"],
                                type(cls).make_property(
                                    cls.__name__, element.attrib["name"], item.attrib
                                ),
                            )
    ...
    def proxy(self):
        if not self._proxy:
            self._proxy = dbus.SystemBus().get_object(
                self.dbus_service, self.object_path, follow_name_owner_changes=True
            )
...
class fixups(object):
                    ...
                    for rule in settings[key]["routing-rules"]:
                        for p in rule:
                            rule[p] = (
                                dbus.Int32(rule[p])
                                if p == "family"
                                else dbus.UInt32(rule[p])
                            )
                    settings[key]["routing-rules"] = dbus.Array(
                        settings[key]["routing-rules"],
    ...
    def to_python(cls, method, arg, val, signature):
        ...
        cls_af = {"IP4Config": socket.AF_INET, "IP6Config": socket.AF_INET6}.get(
            cls, socket.AF_INET
        )
        if method == "Get":
            if arg == "Ip4Address":
