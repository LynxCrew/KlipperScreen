...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.buttons = {
            ...
            "quad_gantry_level": self._gtk.Button(
                "z-tilt", _("Quad Gantry Level"), "color4"
            ),
        ...
        grid.attach(self.buttons["home_xy"], 0, 0, 1, 1)
        ...
        for j, i in enumerate(self.distances):
            ...
            if (self._screen.lang_ltr and j == 0) or (
                not self._screen.lang_ltr and j == len(self.distances) - 1
            ):
                ctx.add_class("distbutton_top")
            elif (not self._screen.lang_ltr and j == 0) or (
                self._screen.lang_ltr and j == len(self.distances) - 1
            ):
    ...
    def process_update(self, action, data):
        ...
        if homed_axes == "xyz":
            if "gcode_move" in data and "gcode_position" in data["gcode_move"]:
                self.labels["pos_x"].set_text(
                    f"X: {data['gcode_move']['gcode_position'][0]:.2f}"
                )
                self.labels["pos_y"].set_text(
                    f"Y: {data['gcode_move']['gcode_position'][1]:.2f}"
                )
                self.labels["pos_z"].set_text(
                    f"Z: {data['gcode_move']['gcode_position'][2]:.2f}"
                )
        else:
            if "x" in homed_axes:
                if "gcode_move" in data and "gcode_position" in data["gcode_move"]:
    ...
    def add_option(self, boxname, opt_array, opt_name, option):
        ...
        if option["type"] == "binary":
            ...
            switch.set_active(
                self._config.get_config().getboolean(option["section"], opt_name)
            )
            switch.connect(
                "notify::active", self.switch_config_option, option["section"], opt_name
            )
            ...
            scale.set_digits(0)
            scale.connect(
                "button-release-event", self.scale_moved, option["section"], opt_name
            )
            dev.add(scale)

        opt_array[opt_name] = {"name": option["name"], "row": dev}
