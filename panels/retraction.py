...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        retract_length = (
            float(conf["retract_length"]) if "retract_length" in conf else 0
        )
        retract_speed = (
            int(float((conf["retract_speed"]))) if "retract_speed" in conf else 20
        )
        unretract_extra_length = (
            float(conf["unretract_extra_length"])
            if "unretract_extra_length" in conf
            else 0
        )
        unretract_speed = (
            int(float((conf["unretract_speed"]))) if "unretract_speed" in conf else 10
        )
    ...
    def set_opt_value(self, widget, event, opt):
        ...
        if opt == "retract_speed":
            self._screen._ws.klippy.gcode_script(
                f"SET_RETRACTION RETRACT_SPEED={value}"
            )
        elif opt == "retract_length":
            self._screen._ws.klippy.gcode_script(
                f"SET_RETRACTION RETRACT_LENGTH={value}"
            )
        elif opt == "unretract_extra_length":
            self._screen._ws.klippy.gcode_script(
                f"SET_RETRACTION UNRETRACT_EXTRA_LENGTH={value}"
            )
        elif opt == "unretract_speed":
            self._screen._ws.klippy.gcode_script(
                f"SET_RETRACTION UNRETRACT_SPEED={value}"
            )
