...
class BasePanel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        self.control["shutdown"].connect(
            "clicked", self.menu_item_clicked, self.shutdown
        )
        self.control["shutdown"].set_no_show_all(True)
        self.control["printer_select"] = self._gtk.Button("shuffle", scale=abscale)
        self.control["printer_select"].connect(
            "clicked", self._screen.show_printer_select
        )
        ...
        self.control["shortcut"].connect(
            "clicked", self.menu_item_clicked, self.shorcut
        )
        ...
        self.action_bar.set_size_request(
            self._gtk.action_bar_width, self._gtk.action_bar_height
        )
    ...
    def show_heaters(self, show=True):
        try:
            ...
            for device in devices:
                ...
                self.labels[f"{device}_box"].pack_start(
                    self.labels[device], False, False, 0
                )
            ...
            if len(self._printer.get_tools()) > (nlimit - 1):
                ...
                if (
                    ...
                    and f"{self.current_extruder}_box" in self.labels
                ):
                    self.control["temp_box"].add(
                        self.labels[f"{self.current_extruder}_box"]
                    )
            else:
    ...
    def get_icon(self, device, img_size):
        ...
        # Extra items
            ...
            return None
        elif device.startswith("temperature_fan") or device.startswith(
    ...
    def add_content(self, panel):
        ...
        for control in ("back", "home"):
            self.set_control_sensitive(
                len(self._screen._cur_panels) > 1, control=control
            )
    ...
    def process_update(self, action, data):
        if action == "notify_proc_stat_update":
            ...
            memory = (
                data["system_memory"]["used"] / data["system_memory"]["total"]
            ) * 100
            ...
            if cpu > 80 or memory > 85:
                ...
                if not ctx.has_class(error):
                    ctx.add_class(error)
                self._screen.log_notification(
                    f"{self._screen.connecting_to_printer}: {msg}", 2
                )
        ...
        if action == "notify_update_response":
            ...
            if "complete" in data and data["complete"]:
                ...
                if self.update_dialog is not None:
                    try:
                        self.update_dialog.set_response_sensitive(
                            Gtk.ResponseType.OK, True
                        )
                        self.update_dialog.get_widget_for_response(
                            Gtk.ResponseType.OK
                        ).show()
        ...
        for device in self._printer.get_temp_devices():
            ...
            if temp is not None and device in self.labels:
                name = ""
                if not (
                    device.startswith("extruder") or device.startswith("heater_bed")
                ):
        ...
        if (
            ...
            self.control["temp_box"].reorder_child(
                self.labels[f"{self.current_extruder}_box"], 0
            )
            self.control["temp_box"].show_all()
    ...
    def show_shortcut(self, show=True):
        show = (
            ...
            and self._printer.get_printer_status_data()["printer"]["gcode_macros"][
                "count"
            ]
            > 0
        )
        ...
        self.set_control_sensitive(
            self._screen._cur_panels[-1] != self.shorcut["panel"]
        )
        self.set_control_sensitive(
    ...
    def set_ks_printer_cfg(self, printer):
        ...
        if self.ks_printer_cfg is not None:
            self.titlebar_name_type = self.ks_printer_cfg.get(
                "titlebar_name_type", None
            )
            titlebar_items = self.ks_printer_cfg.get("titlebar_items", None)
            if titlebar_items is not None:
                self.titlebar_items = [
                    str(i.strip()) for i in titlebar_items.split(",")
                ]
                logging.info(
