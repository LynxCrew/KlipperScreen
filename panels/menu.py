...
class Panel(ScreenPanel):
    ...
    def arrangeMenuItems(self, items, columns=None, expand_last=False):
        ...
        self.autogrid.__init__(
            enabled, columns, expand_last, self._screen.vertical_mode
        )
        return self.autogrid

    def create_menu_items(self):
        count = sum(
            bool(self.evaluate_enable(i[next(iter(i))]["enable"])) for i in self.items
        )
        ...
        for i in range(len(self.items)):
            ...
            if item["panel"]:
                ...
                if item["params"] is not False:
                    try:
                        p = self._screen.env.from_string(item["params"]).render(
                            self.j2_data
                        )
                        params = json.loads(p)
                    except Exception as e:
                        logging.exception(
                            f"Unable to parse parameters for [{name}]:\n{e}"
                        )
                        params = {}
                ...
                if item["confirm"] is not None:
                    ...
                    b.connect(
                        "clicked", self._screen._send_action, item["method"], params
                    )
            else:
    ...
    def evaluate_enable(self, enable):
        if enable == "{{ moonraker_connected }}":
            ...
            return (
                self.ks_printer_cfg
                and self.ks_printer_cfg.get("camera_url", None) is not None
            )
        # self.j2_data = self._printer.get_printer_status_data()
        try:
