...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.wireless_interfaces = [
            iface for iface in self.network_interfaces if iface.startswith("wl")
        ]
        self.wifi = None
        self.use_network_manager = (
            os.system("systemctl is-active --quiet NetworkManager.service") == 0
        )
    ...
    def connect_network(self, widget, ssid, showadd=True):
        isdef = any(
            net["ssid"] == ssid
            for netid, net in self.wifi.get_supplicant_networks().items()
        )
        if not isdef:
            if showadd:
        ...
        self.labels["connecting_info"] = Gtk.Label(
            label=_("Starting WiFi Association"),
            halign=Gtk.Align.START,
    ...
    def update_network_info(self, ssid):
        ...
        if netinfo.get("connected") or self.wifi.get_connected_ssid() == ssid:
            ...
            if ifadd.get(netifaces.AF_INET):
                ...
                self.labels["ip"].set_text(
                    f"IP: {ifadd[netifaces.AF_INET][0]['addr']}  "
                )
            if ifadd.get(netifaces.AF_INET6):
                ipv6 = (
                    f"<b>IPv6:</b> {ifadd[netifaces.AF_INET6][0]['addr'].split('%')[0]}"
                )
            ...
            self.labels["networks"][ssid]["name"].set_label(
                _("Hidden") if ssid.startswith("\x00") else f"{ssid}"
            )
            if "psk" in netinfo:
    ...
    def update_single_network_info(self):
        ifadd = netifaces.ifaddresses(self.interface)
        ipv6 = (
            f"{ifadd[netifaces.AF_INET6][0]['addr'].split('%')[0]}"
            if ifadd.get(netifaces.AF_INET6)
            else ""
        )
