...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.unload_filament = any(
            "UNLOAD_FILAMENT" in macro.upper() for macro in macros
        )
        ...
        self.buttons["temperature"].connect(
            ...
            {"name": "Temperature", "panel": "temperature"},
        )
        self.buttons["spoolman"].connect(
            "clicked", self.menu_item_clicked, {"name": "Spoolman", "panel": "spoolman"}
        )

        self.labels["extruders_menu"] = self._gtk.ScrolledWindow()
        self.labels["extruders"] = Gtk.FlowBox(
            hexpand=True,
            vexpand=True,
            homogeneous=True,
        ...
        if len(filament_sensors) > 0:
            for s, x in enumerate(filament_sensors):
                ...
                self.labels[x]["switch"].connect(
                    "notify::active", self.enable_disable_fs, name, x
                )
                self.labels[x]["box"].pack_start(
                    self.labels[x]["label"], True, True, 10
                )
                self.labels[x]["box"].pack_start(
                    self.labels[x]["switch"], False, False, 0
                )
    ...
    def process_update(self, action, data):
        ...
        if (
            ...
            self.labels[self.current_extruder].get_style_context().add_class(
                "button_active"
            )
            if "current_extruder" in self.labels:
                n = self._printer.get_tool_number(self.current_extruder)
                self.labels["current_extruder"].set_image(
                    self._gtk.Image(f"extruder-{n}")
                )
        ...
        for x in self._printer.get_filament_sensors():
            if x in data:
                ...
                if "filament_detected" in data[x]:
                    self._printer.set_dev_stat(
                        x, "filament_detected", data[x]["filament_detected"]
                    )
    ...
    def enable_disable_fs(self, switch, gparams, name, x):
        if switch.get_active():
            ...
            self._screen._ws.klippy.gcode_script(
                f"SET_FILAMENT_SENSOR SENSOR={name} ENABLE=1"
            )
            ...
            self._screen._ws.klippy.gcode_script(
                f"SET_FILAMENT_SENSOR SENSOR={name} ENABLE=0"
            )
            self.labels[x]["box"].get_style_context().remove_class(
                "filament_sensor_empty"
            )
