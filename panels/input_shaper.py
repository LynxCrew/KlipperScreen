...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        self.freq_xy_adj = {}
        self.freq_xy_combo = {}
        self.calibrate_btn = self._gtk.Button(
            "move", _("Finding ADXL"), "color1", lines=1
        )
        ...
        for i, dim_freq in enumerate(XY_FREQ):
            ...
            self.freq_xy_adj[dim_freq["config"]] = Gtk.Adjustment(
                0, dim_freq["min"], dim_freq["max"], 0.1
            )
            scale = Gtk.Scale(
                adjustment=self.freq_xy_adj[dim_freq["config"]],
                digits=1,
                hexpand=True,
            ...
            scale.connect(
                "button-release-event", self.set_opt_value, dim_freq["config"]
            )
    ...
    def start_calibration(self, widget, method):
        ...
        if method == "x":
            self._screen._send_action(
                ...
                {"script": "SHAPER_CALIBRATE AXIS=X"},
            )
        if method == "y":
            self._screen._send_action(
                self.calibrate_btn,
                "printer.gcode.script",
                {"script": "SHAPER_CALIBRATE AXIS=Y"},
            )
        if method == "both":
            self._screen._send_action(
                self.calibrate_btn,
    ...
    def process_update(self, action, data):
        ...
        if "recommended shaper_type_" in data:
            results = re.search(
                r"shaper_type_(?P<axis>[xy])\s*=\s*(?P<shaper_type>.*?), shaper_freq_.\s*=\s*("
                r"?P<shaper_freq>[0-9.]+)",
                data,
            )
            if results:
                results.groupdict()
            self.freq_xy_adj["shaper_freq_" + results["axis"]].set_value(
                float(results["shaper_freq"])
            )
            self.freq_xy_combo["shaper_type_" + results["axis"]].set_active(
                SHAPERS.index(results["shaper_type"])
            )
            if self.calibrating_axis == results["axis"] or (
                self.calibrating_axis == "both" and results["axis"] == "y"
            ):
                self.calibrate_btn.set_sensitive(True)
