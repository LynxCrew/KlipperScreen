...
class Panel(ScreenPanel):
    ...
    def add_pin(self, pin):
        ...
        if parse_bool(section.get("pwm", "false")) or parse_bool(
            section.get("hardware_pwm", "false")
        ):
            scale = Gtk.Scale.new_with_range(
                Gtk.Orientation.HORIZONTAL, min=0, max=100, step=1
            )
            ...
            self.devices[pin]["switch"].connect(
                "notify::active", self.set_output_pin, pin
            )
