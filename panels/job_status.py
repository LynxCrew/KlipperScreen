...
class Panel(ScreenPanel):
    ...
    def create_status_grid(self, widget=None):
        ...
        buttons["fan"].connect(
            "clicked", self.menu_item_clicked, {"panel": "fan", "name": _("Fan")}
        )
        ...
        for i, extruder in enumerate(self._printer.get_tools()):
            ...
            self.buttons["extruder"][extruder].connect(
                ...
                {"panel": "temperature", "name": _("Temperature"), "extra": extruder},
            )
        ...
        if nlimit > 2 and len(self._printer.get_tools()) == 2:
            for extruder in self.buttons["extruder"]:
                self.labels["temp_grid"].attach(
                    self.buttons["extruder"][extruder], n, 0, 1, 1
                )
                n += 1
        else:
            self.current_extruder = self._printer.get_stat("toolhead", "extruder")
    ...
    def create_buttons(self):
        ...
        self.buttons = {
            ...
            "save_offset_probe": self._gtk.Button(
                "home-z", _("Save Z") + "\n" + "Probe", "color1"
            ),
        ...
        self.buttons["save_offset_endstop"].connect(
            "clicked", self.save_offset, "endstop"
        )
    ...
    def save_offset(self, widget, device):
        ...
        if device == "probe":
            ...
            if "stepper_z" in self._printer.get_config_section_list():
                saved_z_offset = self._printer.get_config_section("stepper_z")[
                    "position_endstop"
                ]
            elif "stepper_a" in self._printer.get_config_section_list():
                saved_z_offset = self._printer.get_config_section("stepper_a")[
                    "position_endstop"
                ]
            if saved_z_offset:
                msg += "\n\n" + _("Saved offset: %s") % saved_z_offset
    ...
    def cancel_confirm(self, dialog, response_id):
        ...
        if response_id == Gtk.ResponseType.APPLY:
            self.menu_item_clicked(
                None, {"panel": "exclude", "name": _("Exclude Object")}
            )
            return
    ...
    def update_time_left(self):
        ...
        progress_type = self._config.get_config()["main"].get(
        ...
        if (
            ...
            and self.file_metadata["estimated_time"] > 1
        ):
        ...
        if (
            ...
            filament_time = print_duration / (
                fila_used / self.file_metadata["filament_total"]
            )
