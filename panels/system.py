...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        self.update_all = self._gtk.Button(
            "arrow-up", _("Full Update"), "color1", self.bts, Gtk.PositionType.LEFT, 1
        )
        self.update_all.connect("clicked", self.show_update_info, "full")
        self.update_all.set_vexpand(False)
        self.refresh = self._gtk.Button(
            "arrow-down", _("Refresh"), "color3", self.bts, Gtk.PositionType.LEFT, 1
        )
    ...
    def create_info_grid(self):
        ...
        for i, prog in enumerate(sorted(list(self.update_status["version_info"]))):
            ...
            try:
                if (
                    prog
                    in self.system_info["result"]["system_info"]["available_services"]
                ):
