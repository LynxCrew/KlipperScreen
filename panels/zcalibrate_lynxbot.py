...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        if self.probe:
            self.x_offset = (
                float(self.probe["x_offset"]) if "x_offset" in self.probe else 0.0
            )
            self.y_offset = (
                float(self.probe["y_offset"]) if "y_offset" in self.probe else 0.0
            )
        ...
        if "axis_twist_compensation" in self._printer.get_config_section_list():
            ...
            if (
                "wait_for_continue" in twist_compensation
    ...
    def set_functions(self):
        ...
        if "BED_MESH_CALIBRATE" in self._printer.available_commands:
            ...
            if "mesh_radius" in mesh:
                ...
                logging.info(
                    f"Mesh Radius: {self.mesh_radius} Origin: {self.mesh_origin}"
                )
            else:
                self.mesh_min = self._csv_to_array(mesh["mesh_min"])
                self.mesh_max = self._csv_to_array(mesh["mesh_max"])
            if "zero_reference_position" in self._printer.get_config_section(
                "bed_mesh"
            ):
    ...
    def start_calibration(self, widget, method):
        ...
        if method == "mesh":
            if (
                self.bed_mesh_calibrate[1]
                and self._printer.get_stat("toolhead", "homed_axes") != "xyz"
            ):
    ...
    def _move_to_position(self, x, y):
        if not x or not y:
            self._screen.show_popup_message(
                _("Error: Couldn't get a position to probe")
            )
            return
    ...
    def _get_probe_location(self):
        ...
        if (
            ...
            return (
                self.mesh_origin[0] - self.x_offset,
                self.mesh_origin[1] - self.y_offset,
            )
    ...
    def _calculate_position(self):
        ...
        try:
            mid_x = (
                float(self._printer.get_config_section("stepper_x")["position_max"]) / 2
            )
            mid_y = (
                float(self._printer.get_config_section("stepper_y")["position_max"]) / 2
            )
        except KeyError:
    ...
    def process_busy(self, busy):
        if self.running:
            for button in self.buttons:
                if button != "start" and (
                    button != "cancel" or not self.twist_compensate_running
                ):
    ...
    def change_distance(self, widget, distance):
        ...
        self.widgets[f"{distance}"].get_style_context().add_class(
            "horizontal_togglebuttons_active"
        )
    ...
    def reset_states(self):
        ...
        if self.start_handler is None:
            if len(self.functions) > 1:
                self.start_handler = self.buttons["start"].connect(
                    "clicked", self.on_popover_clicked
                )
            else:
                self.start_handler = self.buttons["start"].connect(
                    "clicked", self.start_calibration, self.functions[0]
                )
