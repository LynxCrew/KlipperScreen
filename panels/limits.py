...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.options = [
            ...
            },
            {
                ...
                "units": "%",
                "value": (
                    int(float(conf["minimum_cruise_ratio"]) * 100)
                    if "minimum_cruise_ratio" in conf
                    else 50
                ),
                "max": 100,
            },
            ...
            },
            {
                ...
                "units": _("mm/s"),
                "value": (
                    int(float(conf["square_corner_velocity"]))
    ...
    def update_option(self, option, value):
        ...
        for opt in self.options:
            if opt["option"] == option and "max" not in opt:
                if self.values[option] > opt["value"]:
                    self.limits[option]["scale"].get_style_context().add_class(
                        "option_slider_max"
                    )
                    # Infinite scale
                    self.limits[option]["adjustment"].set_upper(
                        self.values[option] * 1.5
                    )
                else:
                    self.limits[option]["scale"].get_style_context().remove_class(
                        "option_slider_max"
                    )
        ...
        self.limits[option]["scale"].connect(
    ...
    def set_opt_value(self, widget, event, opt):
        ...
        if opt == "max_accel":
            ...
            self._screen._ws.klippy.gcode_script(
                f"SET_VELOCITY_LIMIT MINIMUM_CRUISE_RATIO={value / 100}"
            )
            ...
            self._screen._ws.klippy.gcode_script(
                f"SET_VELOCITY_LIMIT SQUARE_CORNER_VELOCITY={value}"
            )
